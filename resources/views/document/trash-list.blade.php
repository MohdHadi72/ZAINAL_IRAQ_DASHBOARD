            ...
            <div class="row">
                <div class="col-sm-12">
                    <div class="page-title-box">
                        ...
                        <h4 class="page-title">Trash List</h4>
                        <a href="{{ url('docs-view') }}"
                            class="btn btn-primary mt-3  float-right">Show-Active</a>
                                ...
                                $.each(response, function(index, role)
                                    {
                                        ...
                                        row.append
                                            (
                                                $('<td>').text(index + 1),
                                                $('<td>').text(role.buyerName),
                                                $('<td>').text(stages),
                                                
                                               $('<td>').append(
                                                     $('<a>').text('Edit').addClass('btn btn-primary m-3').attr('href', "{{ url('/stage/first/edit') }}" + "/" + role + "/" + role.id),
                                                   $('<a>').text('Delete').addClass('btn btn-danger').attr('href', "{{ url('/stage/first/delete') }}" + "/" + role.id)),
                                            );
